...
class NewRequest extends Line
{
    ...
    public function P_query(int $range) {
        ...
        for ($offset = $range - 1; $offset >= 0; $offset--) {
            $array[] = PRequest::whereDate('created_at', today()->subDays($offset))->count();
        }
        return $array;
    }
    ...
    public function S_query(int $range) {
        // 按时间从远到进
        for ($offset = $range - 1; $offset >= 0; $offset--) {
            $array[] = SRequest::whereDate('created_at', today()->subDays($offset))->count();
        }
        return $array;
    }
